...
interface SidebarNavButtonAdminProps {
  href: string;
  label: string;
...
export function SidebarNavButtonAdmin({ icon: Icon, label, href, active }: SidebarNavButtonAdminProps) {
  return (
    <a
      href={href}
      className={cn(
        ...
        active
          ? 'bg-[var(--uaq-terniary-hover)] text-white px-2.5 hover:px-5 group-hover/sidebar:bg-[var(--uaq-terniary-hover)]'
          : 'hover:text-white hover:bg-[var(--uaq-terniary)] px-0 hover:px-2.5'
      )}
    >
        <Icon 
        weight="Bold"
