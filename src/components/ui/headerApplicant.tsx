...
export default function Header({
  ...
  // 🔹 Datos del login (id + token)
  const { token, id: userId } = useApplicantStore();

  // 🔹 Store con la info del usuario
  const { user, fetchUserData } = useUserStore();

  // 🚀 Hidratar user en cuanto haya token + id
  useEffect(() => {
  ...
  const finalTitle = companyTitle || applicantName || 'Aspirante';
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="mono" className="flex items-center gap-2">
                <CompanyAvatar companyName={finalTitle} size="sm" />
